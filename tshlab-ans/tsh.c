...
#define MAXBUF      128   /* max buffer size */
...
eval(char *cmdline) 
{
    ...
    case BUILTIN_NONE:
        ...
        if((pid = fork()) == 0) {
            ...
            //Restore the signals ignored by shell
            ...
            if( tok.infile != NULL )
                if( dup2(fd_in, STDIN_FILENO) < 0 )
                    app_error("IO redirection error");
        ...
        //wait the fg process by a sigsuspend, all signal is handled during waiting time
        sigemptyset(&mask);
        if(!bg)
            while( fgpid(job_list) > 0 )
                sigsuspend(&mask);
        ...
        break;
    default:
        app_error("undefined builtin job");
    }
 ...
 ******************************/
/*
 * safe_printf - a async-signal-safe version printf
 * Copied from the course slide: 14-signals.pdf, p33.
 */
void safe_printf(const char *format, ...) {
    char buf[MAXBUF];
    va_list args;
